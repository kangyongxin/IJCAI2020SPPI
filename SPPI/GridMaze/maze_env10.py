...
class Maze(tk.Tk, object):
    def __init__(self,maze_size,num_hell):
        ...
        self.MAZE_H = maze_size
        self.MAZE_W = maze_size
        self.N_HELL = num_hell
    ...
    def _build_maze(self):
        ...
        for i in range(self.N_HELL):
            ...
            if p==0 and  q==0:
                continue
            if p==self.MAZE_H and q==self.MAZE_H:
                continue
        ...
        # create oval
        oval_center = origin + UNIT *(self.MAZE_H-1)
    ...
    def sit(self,action):
        ...
        if action == 'u':   # up 换乘 down
            base_action[1] += UNIT
        elif action == 'd':   # down => up
            base_action[1] -= UNIT
        elif action == 'l':   # right =>left
            base_action[0] -= UNIT
        elif action == 'r':   # left =>right
            base_action[0] += UNIT
    ...
    def state_space(self):
        S_space = []
        for i in range(self.MAZE_H*self.MAZE_W):
            S_space.append(i+1)
        return S_space
    ...
    def obs_to_state(self,obs):
        ...
        """
        states = ((obs[1] + 15.0 - 20.0) / 40) *self.MAZE_H + (obs[0] + 15.0 - 20.0) / 40+1
        return int(states)

    def state_to_obs(self,stat):
        stat=stat-1
        c1,c0 = stat/self.MAZE_H,stat%self.MAZE_H
        obser=[0,0,0,0]
        obser[0] = c0*40 + 5
        obser[1] = c1*40 + 5
        obser[2] = obser[0] + 30
